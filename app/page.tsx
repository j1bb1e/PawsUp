    ...
    <div className="min-h-screen bg-black p-4 overflow-hidden">
      ...
      >
        <div className="inline-block">
          <h1 className="text-4xl font-bold text-white mb-3">🐾 PawsUp</h1>
        </div>
      </motion.div>
      ...
      <div className="max-w-sm mx-auto relative z-10">
        <AnimatePresence mode="wait">
          ...
          >
            <Card className="overflow-hidden shadow-2xl border border-white/10 bg-gray-900/80 backdrop-blur-xl">
              <div className="relative h-96 perspective-1000">
                ...
                >
                  ...
                  <div className="absolute inset-0 backface-hidden rotate-y-180 bg-gradient-to-br from-gray-800 to-gray-900 p-6 overflow-y-auto">
                    <div className="text-white h-full flex flex-col">
                      <div className="text-center mb-6">
                        <h3 className="text-2xl font-bold mb-2">{displayedPet.name}'s Story</h3>
                      ...
                      <div className="space-y-4 flex-1">
                        <div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm border border-white/20">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {displayedPet.personality.map((trait, index) => (
                              <Badge key={index} className="bg-white/20 text-white border-0 text-xs">
                                {trait}
                              </Badge>
                            ))}
                          </div>
                        ...
                        <div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm border border-white/20">
                          ...
                          <p className="text-sm text-gray-200 leading-relaxed">{displayedPet.quirks}</p>
                        </div>
                        ...
                        <div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm border border-white/20">
                          ...
                          <div className="text-sm text-gray-200">
                            <p className="mb-2">
                              <span className="font-medium">Yard requirement:</span>{" "}
                            ...
                            <p>
                              <span className="font-medium">Energy level:</span> {displayedPet.energy} -
                              {displayedPet.energy === "Low"
                                ? " Perfect for relaxed households"
                                : displayedPet.energy === "Medium"
                                  ? " Enjoys moderate daily activity"
                                  : displayedPet.energy === "High"
                                    ? " Needs lots of exercise and play"
                                    : " Requires very active lifestyle"}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm border border-white/20">
                          ...
                          <p className="text-sm text-gray-200 leading-relaxed">{displayedPet.description}</p>
                        </div>
                      </div>
                ...
                <motion.button
                  ...
                  className="absolute top-4 left-4 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 backdrop-blur-sm transition-all duration-200 border border-white/20"
                  title={isFlipped ? "Show photos" : "Show backstory"}
                >
                  <motion.div animate={{ rotateY: isFlipped ? 180 : 0 }} transition={{ duration: 0.3 }}>
                    {isFlipped ? <Camera className="w-4 h-4" /> : <BookOpen className="w-4 h-4" />}
                  </motion.div>
                </motion.button>
                ...
                >
                  <Badge
                    ...
                    } border-0 px-3 py-1 shadow-lg font-medium`}
                  >
                ...
                <AnimatePresence>
                  {x.get() > 50 && (
                    ...
                    >
                      <div className="bg-green-600 text-white px-8 py-4 rounded-2xl font-bold text-2xl transform rotate-12 shadow-2xl">
                        LOVE 💚
                      </div>
                    </motion.div>
                  )}
                  {x.get() < -50 && (
                    ...
                    >
                      <div className="bg-red-600 text-white px-8 py-4 rounded-2xl font-bold text-2xl transform -rotate-12 shadow-2xl">
                        PASS 💔
                      </div>
                    </motion.div>
                  )}
              ...
              <CardContent className="p-6 bg-gray-900/90 backdrop-blur-sm">
                <motion.div
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h2 className="text-3xl font-bold text-white">{displayedPet.name}</h2>
                      <p className="text-gray-400 text-lg">{displayedPet.breed}</p>
                    </div>
                    <Badge
                      className={`${getEnergyColor(displayedPet.energy)} border border-white/20 px-3 py-1 shadow-lg`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                    <motion.div
                      ...
                      className="flex items-center text-gray-300 bg-white/5 backdrop-blur-sm rounded-lg p-3 border border-white/10"
                    >
                    ...
                    <motion.div
                      whileHover={{ scale: 1.05 }}
                      className="flex items-center text-gray-300 col-span-2 bg-white/5 backdrop-blur-sm rounded-lg p-3 border border-white/10"
                    >
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center text-white">
                        ...
                        <span className="text-xl font-bold">${displayedPet.price}</span>
                        {displayedPet.negotiable && <span className="text-gray-400 text-sm ml-2">(OBO)</span>}
                      </div>
                  ...
                  <p className="text-gray-300 text-sm mb-4 leading-relaxed">{displayedPet.description}</p>
                  ...
                  <motion.div
                    ...
                    className="mb-4 bg-amber-500/10 backdrop-blur-sm rounded-lg p-3 border border-amber-500/20"
                  >
                    <div className="flex items-start gap-2">
                      <AlertCircle className="w-4 h-4 text-amber-400 mt-0.5 flex-shrink-0" />
                      <div>
                        <h4 className="text-amber-400 text-sm font-medium mb-1">Good to Know</h4>
                        <p className="text-gray-300 text-xs leading-relaxed">{displayedPet.quirks}</p>
                      </div>
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="p-2 h-auto text-gray-500 hover:text-gray-300 hover:bg-white/5 bg-transparent backdrop-blur-sm transition-all duration-300 disabled:opacity-30"
                        onClick={handleUndo}
                    ...
                    <Button
                      variant="outline"
                      size="lg"
                      className="flex-1 border-2 border-white/20 text-gray-300 hover:bg-white/5 bg-transparent backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-lg hover:border-white/30"
                    ...
                    <Button
                      size="lg"
                      className="flex-1 bg-white text-black hover:bg-gray-100 border-0 transition-all duration-300 hover:scale-105 hover:shadow-lg shadow-white/10"
      ...
      <motion.div
        ...
        className="text-center mt-8 text-gray-500 text-sm relative z-10"
      >
        <motion.p
